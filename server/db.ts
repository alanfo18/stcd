...
export async function getDb() {
  if (!_db && process.env.DATABASE_URL) {
    try {
      ...
      // Create connection configuration with SSL
      const connectionConfig = {
        ...
        ssl: 'amazon', // Use 'amazon' for TiDB Cloud
        waitForConnections: true,
        connectionLimit: 10,
        queueLimit: 0,
      };
      
      console.log('[Database] Connecting to:', connectionConfig.host + ':' + connectionConfig.port);
...
export async function getDiaristasForUser(userId: number): Promise<any[]> {
  ...
  try {
    // Get all diaristas (this is a placeholder - adjust based on your actual relationship)
    return await db.select().from(diaristas);
  } catch (error) {
    console.error("[Database] Error getting diaristas:", error);
    return [];
...
// Import helpers at the end to avoid circular dependencies
